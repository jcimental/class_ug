...
int background_w_fld(
  ...
  case JJE:
    *dw_over_da_fld = pba->w0_0*pba->w0_1*pba->w0_2*cos(pba->w0_3 + 1./a -1.)*pow(1./a -1.,pba->w0_2 -1.)/(pow(1.+pba->w0_1*pow(1./a -1.,pba->w0_2),2)*pow(a,2))
    ...
    break;
  case LINEAL:
    *dw_over_da_fld = pba->w0_lin2/pow(a,2);
    break;
  case BA:
    *dw_over_da_fld = pba->w0_ba2*(2.*pow(a,2.)-4.*a+1)/pow(4.*a-2.,2);
    break;
  }
  ...
  case JJE:
    *integral_fld = 0.;
    break;
  case LINEAL:
    *integral_fld = 3.*(pba->w0_lin2*(1.-1./a)-(1.+pba->w0_lin1+pba->w0_lin2)*log(a));
    break;
  case BA:
    *integral_fld = -3.*(log(a)*(1.+pba->w0_ba1+pba->w0_ba2)+pba->w0_ba2*log(2.*pow(a,2)-2.*a+1)/2);
    break;
  }
