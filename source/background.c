...
int background_functions(
  ...
  /* extra fluid with time-independent equation of state parameter*/
  double w_efld, intergal_efld;
  ...
  if (pba->has_efld == _TRUE_) {
    pvecback[pba->index_bg_rho_efld] = pvecback_B[pba->index_bi_rho_efld];
    pvecback[pba->index_bg_rho_efld_t] = pba->Omega0_efld * pow(pba->H0,2)/pow(a,3.*(1.+pba->w0_efld));
    class_call(background_w_efld(pba,a,&w_efld,&dw_over_da,&intergal_efld),pba->error_message, pba->error_message);
    pvecback[pba->index_bg_w_efld] = w_efld;

    rho_tot += pvecback[pba->index_bg_rho_efld];
    p_tot += w_efld * pvecback[pba->index_bg_rho_efld];
  }
...
int background_w_fld(
  ...
  case EDE:
    ...
    *dw_over_da_fld = - d2Omega_ede_over_da2*a/3./(1.-Omega_ede)/Omega_ede
    ...
    break;
  case JJE:
    *dw_over_da_fld = pba->w0_0/pow(a,2)*(pba->w0_1*pba->w0_2*cos(pba->w0_3+1./a-1.)*pow(1./a-1.,pba->w0_2-1)/pow(1+pba->w0_1*pow(1./a-1.,2),2)+sin(pba->w0_3+1./a-1.)/(1+pba->w0_1*pow(1./a-1,pba->w0_2)));
    break;
  }
...
int background_w_efld(
		      struct background *pba,
		      double a,
		      double * w_efld,
		      double * dw_over_da_efld,
		      double * integral_efld
		      ) {
  *w_efld = pba->w0_efld;
  *dw_over_da_efld = 0.;
  *integral_efld = -3.*(1. + pba->w0_efld)*log(a);
  return _SUCCESS_;
}
...
int background_indices(
  ...
  class_define_index(pba->index_bg_w_efld,pba->has_efld,index_bg,1);
  class_define_index(pba->index_bg_rho_efld_t,pba->has_efld,index_bg,1);
  ...
  /* -> energy density in extra fluid */
  class_define_index(pba->index_bi_rho_efld,pba->has_efld,index_bi,1);
...
int background_initial_conditions(
  ...
  double rho_efld_today;
  double w_fld,dw_over_da_fld,integral_fld;
  double w_efld,dw_over_da_efld,integral_efld;
  ...
  */
  ...
  if (pba->has_efld == _TRUE_) {
    /* rho_efld today */
    rho_efld_today = pba->Omega0_efld * pow(pba->H0,2);    
    
    /* integrate rho_efld(a) from a_ini to a_0, to get rho_efld(a_ini) given rho_efld(a0) */
    class_call(background_w_efld(pba,a,&w_efld,&dw_over_da_efld,&integral_efld), pba->error_message, pba->error_message);
    
    /* rho_efld at initial time */
    pvecback_integration[pba->index_bi_rho_efld] = rho_efld_today * exp(integral_efld);
  }
...
int background_output_titles(
  ...
  class_store_columntitle(titles,"(.)rho_efld_t",pba->has_efld);
  class_store_columntitle(titles,"(.)w_fld",pba->has_fld);
  class_store_columntitle(titles,"(.)w_efld",pba->has_efld);
...
int background_output_data(
  ...
  for (index_tau=0; index_tau<pba->bt_size; index_tau++) {
    ...
    class_store_double(dataptr,pvecback[pba->index_bg_rho_efld_t],pba->has_efld,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_w_fld],pba->has_fld,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_w_efld],pba->has_efld,storeidx);
...
int background_derivs(
  ...
  if (pba->has_efld == _TRUE_) {
    /** - Compute efld density \f$ d\rho/dloga = -3 (1+w_{efld}(a)) \rho \f$ */
    dy[pba->index_bi_rho_efld] = -3.*(1.+pvecback[pba->index_bg_w_efld])*y[pba->index_bi_rho_efld];
  }
...
int background_output_budget(
  ...
  if (pba->background_verbose > 1) {
    ...
    /**if (pba->has_efld == _TRUE_) {
      class_print_species("Second Dark Energy Fluid",efld);
      budget_other+=pba->Omega0_efld;
